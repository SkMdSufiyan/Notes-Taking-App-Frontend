...
import NotesTakingContext from './NotesTakingContext';
import { useNavigate } from 'react-router-dom';

const NotesTakingProvider = (props) => {
    ...
    // After that the this "NotesTakingProvider" will monitor the localStorage for accessToken
...
// A function to update the "accessToken" as well as "userProfile" from localStorage
    ...
    useEffect(() => {
      ...
      // Clearing the interval (when the "NotesTakingProvider" component is unmounted)
...
const logoutUserFun = () => {
  ...
  return (
    <NotesTakingContext.Provider value={{accessToken, setAccessToken, userProfile, setUserProfile, apiCallError, setApiCallError, logoutMessage, setLogoutMessage, logoutUserFun, config, serverUrl}}>
        {props.children}
    </NotesTakingContext.Provider>
  )
}

export default NotesTakingProvider;
